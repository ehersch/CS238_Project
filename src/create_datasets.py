...
# Paths
DATA_DIR = "/Users/andrewsung/Developer/CS238_Project/data"
SRC_DIR = "/Users/andrewsung/Developer/CS238_Project/src"
...
def load_base_data(contract_type):
    ...
    futures = pd.read_csv(f"{DATA_DIR}/oil_data/NYMEX_futures/Cushing_OK_Crude_Oil_Future_Contract_{contract_type}.csv")

    df = pd.merge(spot, futures, on="Date", how="inner")
    df = df.rename(columns={"Price_y": "CL1", "Price_x": "spot_price"})

    # Pipeline weather
    pipeline_locations = ["alaska", "bakken", "western_canada", "midwest_corridor",
                          "ok_hub", "permian", "rockies", "texas_gulf"]
    for loc in pipeline_locations:
        ...
        loc_df = loc_df.rename(columns={
            "mean_temp_K": f"{loc}_temp",
            "total_precip_mm": f"{loc}_precip"
        })
        df = pd.merge(df, loc_df, on="Date", how="left")

    # Producing region weather
    producing_locations = ["bakken", "canada", "eagle", "central_kansas", "n_alaska",
                           "nc_ok", "ne_ok", "nw_ok", "permian", "s_alaska",
                           "saskatchewan", "se_kansas", "sw_kansas"]
    for loc in producing_locations:
        loc_df = pd.read_csv(f"{DATA_DIR}/processed_data/producing_locations/{loc}_producing.csv")
        loc_df = loc_df.rename(columns={
    ...
    # Hurricane data
    hurricane_df = pd.read_csv(f"{DATA_DIR}/weather_data/producing_locations/hurdat2_processed_cleaned.csv")
...
def analyze_negative_price_period(df_log, df_simple):
    ...
    print(f"If you went LONG on April 20: reward = +1 * {apr20_return:.4f} = {apr20_return:.4f}")
    print(f"If you went SHORT on April 20: reward = -1 * {apr20_return:.4f} = {-apr20_return:.4f}")
...
def main():
    ...
    for contract in range(1, 5):
        ...
        print(f"    Shape: {df_log.shape}, NaN in ret_CL1: {df_log['ret_CL1'].isna().sum()}")
        ...
        print(f"    Shape: {df_simple.shape}, NaN in ret_CL1: {df_simple['ret_CL1'].isna().sum()}")
