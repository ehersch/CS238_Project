...
def run_comparison(train_pct=0.8):
    print("=" * 70)
    print(
        f"COMPARING LOG RETURNS vs SIMPLE RETURNS MODELS ({int(train_pct*100)}/{int((1-train_pct)*100)} split)"
    )
    print("=" * 70)
    ...
    feature_cols = [
        c for c in df_log.columns if c not in ["Date", "ret_CL1", "spot_price", "CL1"]
    ]
    return_col = "ret_CL1"
    ...
    for name, df in [("Log Returns", df_log), ("Simple Returns", df_simple)]:
        ...
        print(
            f"Training samples: {len(train_valid)} (dropped {len(train_df) - len(train_valid)} NaN)"
        )
        print(
            f"Test samples: {len(test_valid)} (dropped {len(test_df) - len(test_valid)} NaN)"
        )
        ...
        training_rewards = pomdp.train_qlearning(
            train_df, feature_cols, return_col, n_episodes=50, alpha=0.1
        )

        # Evaluate
        eval_results = pomdp.evaluate(
            test_df, feature_cols, return_col, use_qlearning=True
        )
    ...
    if apr_mask.any() and apr_mask.idxmax() < train_size:
        ...
        extreme_return = df_simple.loc[
            df_simple["Date"] == "2020-04-20", "ret_CL1"
        ].values
