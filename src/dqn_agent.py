...
class DQNAgent:
    ...
    def _linear_epsilon_decay(
        self, step: int, epsilon_start: float, epsilon_end: float, epsilon_decay_steps: int
    ) -> float:
    ...
    def train(
        ...
        for episode in range(n_episodes):
            ...
            while not (done or truncated):
                ...
                if max_steps_per_episode is not None and steps_in_episode >= max_steps_per_episode:
                    break
    ...
    def evaluate(self, env) -> Dict[str, Any]:
        ...
        # track current position inferred from action:
        # env action: 0=SHORT,1=FLAT,2=LONG -> position = action - 1
        while not (done or truncated):
        ...
        sharpe = (
            mean_reward / (std_reward + 1e-8) * np.sqrt(252)
